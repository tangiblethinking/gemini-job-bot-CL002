...
// ─── Client-side title parser (mirrors server-side, for cached jobs missing company) ──
function parseJobTitle(raw: string): { title: string; company: string } {
  ...
  if (ghMatch) return { title: ghMatch[1].trim(), company: ghMatch[2].trim() };
  ...
  // Pattern 4: "[ROLE] - [COMPANY]" or "[ROLE] — [COMPANY]"
  const dashMatch = title.match(/^(.+?)\s+[-–—]\s+([A-Z][^\-–—]+)$/);
  if (dashMatch) return { title: dashMatch[1].trim(), company: dashMatch[2].trim() };

  // Clean noise, no company found
  title = title.replace(/\s*\(remote\)/gi, '').replace(/\s*\(hybrid\)/gi, '').trim();
  return { title, company: '' };
}

// Resolve company + title for a job — handles cached (no company field) and fresh
function resolveJob(job: Job): { title: string; company: string; overline: string } {
  if (job.company) {
    return { title: job.title, company: job.company, overline: job.company };
  }
  // Cached job — parse client-side
  const parsed = parseJobTitle(job.title);
  return {
    title: parsed.title || job.title,
    company: parsed.company,
    overline: parsed.company || job.source,
  };
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#fff', display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: '#1a1a1a', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', gap: 10, flexShrink: 0 }}>
        <button
          onClick={onClose}
          style={{ background: 'rgba(255,255,255,0.12)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '6px 14px', fontSize: 13, cursor: 'pointer' }}
        >
          ← Back to Results
        </button>
        <button
          onClick={() => window.print()}
          style={{ background: '#007aff', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
        >
          Save as PDF
        </button>
      </div>
      <div style={{ flex: 1, overflow: 'auto', background: '#fff' }} dangerouslySetInnerHTML={{ __html: extractBodyForOverlay(html) }} />
    </div>
  );
}

function extractBodyForOverlay(html: string): string {
  const styleMatch = html.match(/<style[^>]*>([\s\S]*?)<\/style>/i);
  let styles = styleMatch ? `<style>${styleMatch[1]}</style>` : '';
  const bodyMatch = html.match(/<body[^>]*>([\s\S]*)<\/body>/i);
  let body = bodyMatch ? bodyMatch[1] : html;
  body = body.replace(/<div class="action-bar">[\s\S]*?<\/div>/, '');
  body = body.replace(/margin-top:\s*\d+px/, 'margin-top: 0');
  return styles + body;
}
...
<img src="https://cdn.myportfolio.com/abc1e0ab-7370-4502-8c78-92428397bf66/15a26956-0efa-4a8d-be26-27a030f18db9.png?h=6a5aea5f291cc6ed6573f41e6a765bf2" alt="Ape X"/>
<p>Resume generation failed.</p><small>Please close this tab and try again.</small>
<button class="b" onclick="window.close()">Close Tab</button>
</body></html>`;
...
// Open HTML in a new tab using blob URL — avoids document.write cross-origin issues
// and allows window.close() to work reliably from the opened tab
function openHtmlInTab(html: string): Window | null {
...
export default function JobList({ jobs }: { jobs: Job[] }) {
  ...
  async function runATS(job: Job): Promise<string> {
    ...
    const atsRes = await fetch('/api/ats', {
      ...
      body: JSON.stringify({
        ...
        snippet: job.snippet,
        parsedResume,
        jobTitle: resolved.title,  // always clean title, never raw
      }),
    });
  ...
  // DESKTOP: blob URL → new tab. window.close() in generated HTML works because
  // the tab was opened programmatically by this page.
  function handleDesktop(job: Job) {
    // Open loading screen immediately via blob
    const loadingTab = openHtmlInTab(LOADING_HTML(resolveJob(job).title));
    ...
    runATS(job)
      .then(html => {
        ...
        // Navigate the already-open tab to the result blob URL
        if (loadingTab && !loadingTab.closed) {
          const blob = new Blob([html], { type: 'text/html' });
          const url = URL.createObjectURL(blob);
          loadingTab.location.href = url;
          setTimeout(() => URL.revokeObjectURL(url), 60000);
        } else {
          // Tab was closed — open fresh
          openHtmlInTab(html);
        }
      })
      .catch(() => {
        if (loadingTab && !loadingTab.closed) {
          const blob = new Blob([ERROR_HTML], { type: 'text/html' });
          const url = URL.createObjectURL(blob);
          loadingTab.location.href = url;
  ...
  // MOBILE: in-page overlay — popstate handles back button
  function handleMobile(job: Job) {
    ...
    setOverlayHtml(
      `<div style="display:flex;align-items:center;justify-content:center;height:100%;font-family:-apple-system,sans-serif;color:#555;font-size:14px;flex-direction:column;gap:12px;padding:24px;text-align:center">
        ...
        <small style="color:#999;font-size:12px">${resolved.title}</small>
      </div>`
    );
    runATS(job)
      .then(html => {
    ...
    <>
      ...
      <div className="flex flex-col gap-3">
        {jobs.map((job, i) => {
          ...
          const { title, overline } = resolveJob(job);
